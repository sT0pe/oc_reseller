									...
									<?php if(!empty($message['attachments'])) { ?>
										<div class="attachments">
											<?php foreach($message['attachments'] as $attachment) { ?>
												<a href="<?php echo $this->url->link('multimerch/conversation/downloadAttachment', 'code=' . $attachment['code'] . '&token=' . $this->session->data['token'], true); ?>"><i class="fa fa-file-o" aria-hidden="true"></i> <?php echo $attachment['name']; ?></a>
												<br/>
											<?php } ?>
										</div>
									<?php } ?>
	...
	$(document).ready(function() {
		$("#ms-message-reply").click(function() {
			...
			$.ajax({
				...
				beforeSend: function() {
					$('#ms-message-form #ms-message-reply').hide().before('<span class="wait">&nbsp;<img src="view/image/loading.gif" alt="" /></span>');
				},
				success: function(jsonData) {
					$('.error').text('');
					if (!jQuery.isEmptyObject(jsonData.errors)) {
						$('#ms-message-form #ms-message-reply').show().prev('span.wait').remove();
